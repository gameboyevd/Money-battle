...
async def on_ready():
    ...
    # 봇 재시작 후 playing 게임 탈락 루프 재개
    try:
        await resume_playing_elimination_loops()
    except Exception as e:
        print(f"resume elimination error: {e}")


async def resume_playing_elimination_loops():
    """status=playing 인 게임의 탈락 타이머를 다시 시작"""
    connection = await get_db()
    try:
        rows = await connection.fetch(
            """
            SELECT id, channel_id, game_data
            FROM games
    ...
    for row in rows:
        ...
        if channel is None:
            try:
                ...
                print(f"resume: channel {ch_id} fetch fail: {e}")
                continue
        gd = parse_game_data(row.get("game_data"))
        # next_elimination_at 없으면 지금+주기로 보정
        if not gd.get("next_elimination_at"):
            settings = merge_game_settings(gd)
            interval = int(settings.get("elimination_interval", DEFAULT_ELIMINATION_INTERVAL))
            gd = dict(settings)
            gd["next_elimination_at"] = (
            ...
            try:
                await connection.execute(
                    "UPDATE games SET game_data = $2::jsonb WHERE id = $1",
                    game_id, json.dumps(gd)
                )
            finally:
                await connection.close()
        start_elimination_task(game_id, channel)
        resumed += 1
        print(f"Resumed elimination loop for game {game_id} channel {ch_id}")
    print(f"Resumed {resumed} playing game elimination loop(s)")
