...
async def get_test_game_screen(game_id, user_id):
...
class WaitingView(discord.ui.View):
    ...
    async def cancel(self, interaction, button):
        ...
        async with lock:
            try:
                # 반드시 waiting 게임만 조회한다.
                game_check = await get_waiting_game(
                    interaction.channel.id
                )

                if not game_check or game_check["id"] != self.game_id:
                    await interaction.response.send_message(
                        "🔒 현재 참가 취소가 가능한 대기 게임이 없습니다.",
                        ephemeral=True
                    )
                    return
    ...
    async def start(self, interaction, button):
        ...
        async with lock:
            try:
                ...
                # DB FOR UPDATE + status='waiting' 검사로
                # 실제 시작 자체도 중복 실행을 막는다.
                started_count = await start_survival_game(
                    game["id"]
                )
        ...
        async with lock:
            try:
                ...
                for job_name, job in JOBS.items():
                    embed.add_field(
                        name=f"{job['emoji']} {job_name}",
                        value=f"{job['reward']:,} 코인",
                        inline=True
                    )

                embed.set_footer(
                    text=f"현재 코인: {player['money']:,}"
                )

                await interaction.response.send_message(
                    embed=embed,
                    view=JobView(self.game_id),
                    ephemeral=True
                )
...
async def force_end_game_command(interaction):
    ...
    async with lock:
        try:
            ...
            view = ForceEndConfirmView(game["id"])
...
class MainView(discord.ui.View):
    ...
    async def my_info(self, interaction, button):
...
async def game_test(interaction):
    ...
    async with lock:
        try:
            ...
            game_row, player, count, alive_count = (
